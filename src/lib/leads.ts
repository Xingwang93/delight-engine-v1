...
export const FORMAT_GYM = "In-Person Training";
export const FORMAT_RECOMMEND = "Let the coach recommend";
...
export const TRAINING_FORMATS = [
  ...
  { title: FORMAT_GYM, subtitle: "1-on-1 sessions with your coach at the gym." },
...
/** Fixed-price plans the coach picks from. */
...
/** "At gym" / "Online" label from preference; null for "let the coach recommend". */
...
/** Payment instruction line for a plan's lane. */
export function paymentLine(plan: Plan | null): string {
  return plan?.lane === "online"
    ? `→ I'll send you the payment details in my next message.`
    : `→ Your first payment is made in person at the gym — bring it to your first session.`;
}
...
export function buildWhatsAppMessage(
  ...
  return [
    ...
    ``,
    `Thanks for reaching out — I reviewed your answers and this is the plan I recommend for you.`,
    ``,
    `Your plan: ${planName}`,
    `Price: ${amount}`,
    ``,
